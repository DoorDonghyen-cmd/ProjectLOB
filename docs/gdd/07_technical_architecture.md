...
@export var caliber: Enums.Caliber
...
@export var passive_acc_bonus: int          # 전탄 ACC 가산
```
...
enum Caliber { CAL_9MM, CAL_556, CAL_762 }
enum BulletEffect { NONE, ARMOR_SHRED, COMBO, LAST_SHOT, OPENING_SHOT, CALIBER_DIFF, PIERCE }
enum EnemyArchetype { RUSHER, TANK, DODGER, CASTER }
enum EnemyStance { NONE, IRON_SHIELD, ACTIVE_DODGER }
```
...
| | `dmr.tres` | 지정사수소총: 4발 스택 / 리로드 1턴 / 약실 있음 / ACC +1 패시브 |
| **적** | `rusher.tres` | 돌격 요원 (Rusher): SPD 3 / DEF 1 / HP 8 / EVA 2 — 빠른 속도 돌격병 |
...
스키마 v2부터 `MaintenanceOverlay`가 상점 최초 진열·리롤·구매를 비전투
타임라인 이벤트로 추가한다. 각 이벤트는 당시 런 문맥, 후보 스냅샷,
빌드 분기, 구매 전후 크레딧을 함께 저장하므로 전투 결과와 선택 원인을
같은 런 파일에서 연결할 수 있다.
