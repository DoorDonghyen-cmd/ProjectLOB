...
## 인카운터를 시작한다. 총과 적 데이터 배열 및 렐릭 목록을 받아 초기화.
func start_encounter(gun_data: GunData, enemy_datas: Array[EnemyData], relics: Array[String] = []) -> void:
	gun = gun_data
	enemies.clear()
	var offset := 0
...
func _fire_internal(target: EnemyInstance) -> void:
	...
	# ── 1. 명중 판정 ──
	var hit := DamageCalculator.check_hit(bullet, target.current_evasion, gun)
	last_shot_hit = hit
	...
	if hit:
		...
		# ── 5. 넉백 ──
		var kb := DamageCalculator.calculate_knockback(bullet, gun)
		...
		# ── 6. 둔화 ──
		if bullet.slow > 0:
			target.apply_slow(bullet.slow)
			combat_log.emit("   ↳ 둔화 -%d (다음 턴)" % bullet.slow)
