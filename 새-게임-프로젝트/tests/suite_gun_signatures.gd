...
static func run(t) -> void:
	...
	# 샷건은 패시브 넉백 +1이 있어 자기 사격으로 적을 초근접 범위 밖으로 밀어낸다.
	#   1발: 거리2 → 점블 +4 = 7 대미지 → 넉백으로 거리3
	#   2발: 거리3 → 보너스 없음 = 3 → 거리4
	#   3발: 거리4 → 원거리 ACC-4 페널티(EVA0라 명중 유지) = 3
	t.eq(sg_close, 47, "샷건 초근접: 첫 발 +4 후 자체 넉백으로 범위 이탈 → 총 13 대미지 (HP 60→47)")
