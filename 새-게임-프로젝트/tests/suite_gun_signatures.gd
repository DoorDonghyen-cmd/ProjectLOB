...
static func run(t) -> void:
	...
	t.eq(rev_close, 50, "리볼버 초근접: 3x3 + 집중1 = 10 대미지")
	...
	t.eq(stalker_rev, 4, "광학 추적체 03: 리볼버 최대 ACC8 < EVA9 → 전탄 빗나감 (HP 4 불변)")
	var stalker_dmr := _hp_after(G_DMR, load("res://resources/enemies/nano_stalker.tres").duplicate() as EnemyData, 3, 8, 0, 3)
	t.check(stalker_dmr < 4, "광학 추적체 03 카운터: DMR 저격 시그니처로 명중 성공 (HP %d < 4)" % stalker_dmr)
