...
## 2026-09-02 (Wed) - 랜덤 패 체감·탄환 카드 가독성 1차 구현 마감

### 🎯 목표
- 고정 비교 시드 때문에 랜덤 패가 반복되어 보이던 문제를 해결하고, 셔플·공개·보충이 실제 화면 사건으로 느껴지게 한다.
- 탄환의 원시 수치보다 명중·관통·화력·제어 역할과 현재 전투 효용을 먼저 읽게 한다.

### 🛠️ 주요 작업
- 고정 시드 A/B 비교와 실행마다 신규 시드를 쓰는 `B 체감 · 매번 새 패`를 분리하고, 실제 시드를 플레이테스트 보고서와 QA 상태에 기록했다.
- 패 전환 데이터에 `initial / refill / reshuffle`, 미사용 `retained`, 신규 `added`, 미공개 수와 다음 2발 예고를 추가했다.
- 전투 시작 셔플·7장 순차 공개, 리로드 유지/신규 보충, 버린 전술탄 재혼합 오버레이를 구현하고 연출 중 입력 잠금 및 전투 교체 시 잠금 정리를 적용했다.
- 탄환 카드를 `◎ 명중 / ◆ 관통 / ✦ 화력 / ↔ 제어` 역할, 핵심 결과 문장, 발동 조건→결과, 현재 대상 명중/관통 판정 중심으로 개편했다.
- 기존 탄환 PNG를 48px 이상·완전 불투명·Nearest 필터의 주 이미지로 승격하고, 공개/미공개/다음 보충 영역과 2발 개별 예고 칩을 추가했다.
- 개발자 메뉴에 고정 B 비교, 랜덤 B 체감, 패 보충 연출 QA, 탄환 카드 가독성 QA 진입점을 분리했다.
- 전투 수치, 공개 패 7발, 기본탄 보장, 결정론적 LIFO는 변경하지 않았다.
- UI 제작 기획서·탄환 랜덤성 설계서·태스크 트래커를 `1차 구현 완료 · 사람 플레이 검증 대기`로 동기화했다.
- 사용자 개인 `.claude/settings.local.json`은 커밋 범위에서 제외했다.

### ✅ 검증
- Godot 4.7 전체 자동 검증 **3,655 통과 / 0 실패 / 기존 경보 3**.
- 122개 스크립트 컴파일과 4개 씬 로드를 확인했다.
- 신규 시드, 패 전환 분류, 후보탄 현재 대상 계산, 공개 연출, 다음 보충 칩, 카드 역할·문장·이미지와 기존 기관단총·전투 입력 회귀를 확인했다.
- `git diff --check`를 통과했다.

### 📅 다음 예정 작업
- 사용자가 `B 체감 · 매번 새 패`를 최소 3회 플레이해 시작 패 변화가 설명 없이 인지되는지 확인한다.
- 탄환 10개 중 8개 이상을 3초 내 역할로 구분할 수 있는지, 현재 적 명중/관통 여부를 5초 내 판단할 수 있는지 확인한다.
- 리로드 뒤 유지/신규 탄환 7발 중 6발 이상을 구분하고 반복 피로를 5점 만점으로 기록한다.
- 사람 결과 뒤에만 공개 패 크기 조정, 신규 탄환 이미지 제작, 정식 B안 채택 여부를 결정한다.

---

## 2026-08-31 (Mon) - 탄환 랜덤 공개 패 A/B 프로토타입·비교 가독성 구현

### 🎯 목표
- 고정 전술탄 전체 접근으로 같은 최적 조합이 반복되는 문제를, 공개 후보만 seeded random으로 제한하는 비파괴 A/B 실험으로 검증 가능하게 만든다.
- A/B 규칙 차이가 장전·전투 화면에서 즉시 읽히도록 비교 UI를 보강한다.

### 🛠️ 주요 작업
- 프로덕션 기본 A안은 유지하고, B안에 12발 덱→공개 패 7발, 미사용 유지, 리로드 부족분 보충, 다음 2발 예고, 기본탄 상시 보장을 구현했다.
- 패 밖 탄환을 UI·블랙박스 QA 양쪽에서 선택하지 못하게 하고, 장전 후 LIFO·명중·관통·피해·적 정보의 결정론은 유지했다.
- 개발자 메뉴에 같은 덱·적·시드 `731042`를 쓰는 A/B 즉시 진입점을 추가했다.
- 초기 패·예고·리로드 전후 패를 플레이테스트 보고서에 기록하고 QA 공개/오라클 상태에 variant를 연결했다.
- 사용자 피드백에 따라 A 파란 `12발 전체 자유 선택`, B 노란 `랜덤 7발 제한` 고정 배너를 추가하고 버튼·HUD·가방 탭·내부 설명을 분리했다.
- 설계 문서와 태스크 트래커를 `구현 완료·사람 플레이 검증 대기` 상태로 갱신했다.
- 사용자 개인 `.claude/settings.local.json`은 작업 범위에서 제외했다.

### ✅ 검증
- Godot 4.7 전체 자동 검증 **3,629 통과 / 0 실패 / 기존 경보 3**.
- 122개 스크립트 컴파일, 4개 씬 로드.
- A/B 실제 숏컷, 고정 배너, B 패 7발·예고 2발·보고서 variant, seed 재현, 패 밖 거부, 기본탄 보장, LIFO·미사용 유지·리로드 보충 회귀를 확인했다.
- `git diff --check`를 통과했다.

### 📅 다음 예정 작업
- 개발자 테스트에서 A 3회·B 3회 직접 플레이하고 조합 고민·성공감·반복감·장전 피로를 1~5점으로 비교한다.
- B안이 단순 제한이 아니라 다른 유효 조합을 만들었는지 확인한다.
- 체감 차이가 약하면 공개 패 7→5~6 후보를 비교하고, 사람 결과 후에만 정식 규칙/GDD 채택 여부를 결정한다.
- A/B 보고서에서 동일 순서 반복률·조합 종류·기본탄 비율·의미 있는 선택 수를 자동 비교하는 후속 집계를 구현한다.
- 현재 변경 사항은 커밋·푸시하지 않은 상태다.

---

## 2026-08-30 (Sun) - 실제 플레이 QA 팀·대시보드·LIFO 핵심 재미 판정 완성

### 🎯 목표
- QA가 단순 회귀 통과율을 보여주는 데 그치지 않고 실제 게임을 플레이해 버그와 재미를 분리 검증하며, LIFO 장전 순서와 탄환 조합의 핵심 재미를 객관적인 근거로 설명하도록 완성한다.

### 🛠️ 주요 작업
- 기능 QA 리드, 블랙박스 경험 테스터, 결정론 전투 시뮬레이터의 독립 역할·입력·보고 계약과 실제 메인 씬 상태/행동 브리지를 구축했다.
- 공개 정보만 사용하는 4성향 자동 플레이, 행동 전 기대·이유·대안 기록, UI 체크포인트, RNG 재현, 제품 버그 2/2 재현 규칙을 구현했다.
- QA 실행·취소·진행률과 격리 세이브를 제공하는 localhost 컨트롤러, 실행 이력 HTML 대시보드, 더블클릭 Windows 런처 `ProjectLoB-QA.exe`를 추가했다.
- 동일 탄환 묶음의 계획 순서·역순·기본탄을 실제 `CombatManager`에서 비교하는 네 가지 핵심 재미 probe를 추가했다.
- 순서 영향, 상황별 해법, 혼합 장전 가치, 실제 선택 압력, 표본 범위를 총점 없이 독립 판정하고 자동화할 수 없는 타격감·피로도·재도전 욕구를 사람 확인으로 분리했다.
- 탄환 내부 ID와 표시명 혼용, 탄환별 장전 횟수의 전체 선택 집중도 오집계, 총 전투 수 표기 오류를 수정했다.
- 대시보드 상단에 사람 언어의 핵심 결론·근거·다음 검증을 배치하고 계획·워크스루·태스크 트래커·히스토리 마스터를 동기화했다.
- 사용자 개인 설정 `.claude/settings.local.json`은 개인 PC 경로·권한 정보이므로 작업 커밋에서 제외했다.

### ✅ 검증
- Godot 4.7 전체 자동 검증 **3,599 통과 / 0 실패 / 기존 경보 3**.
- 통제 시나리오에서 순서 영향 4/4, 계획 순서 우세 4/4, 혼합 장전 우세 4/4를 확인했다.
- 실제 전체 QA 실행 `qa-20260830T084101`이 컨트롤러 `completed`, 신규 확정 버그 0으로 종료됐다.
- 실제 결과는 1개 시드·9전투 표본을 이유로 `PROMISING_BUT_THIN`으로 보수 판정했다.
- 대시보드 JavaScript와 QA PowerShell 구문 검사, `git diff --check`를 통과했다.

### 📅 다음 예정 작업
- 최소 3개 시드·12전투 이상의 반복 실행으로 캠페인 전반의 조합 선택 빈도와 깊이를 재검증한다.
- 사람이 직접 장전 고민의 즐거움, 결산 성공의 타격감, 장시간 계산 피로와 재도전 욕구를 기록한다.
- 반복 근거와 사람 체감이 모두 충족될 때만 핵심 재미 판정을 `CORE_FUN_CONFIRMED`로 승격한다.

---

## 2026-08-21 (Fri) - 조합탄 전문축 가독성·피해 증폭탄 마감

### 🎯 목표
- 8월 17일 구현 후 남아 있던 조합탄 전문축 가독성 개선과 피해 증폭탄 변경분을
  문서·회귀 결과와 함께 최종 정리하고 원격 저장소에 동기화한다.

### 🛠️ 주요 작업
- 전술탄을 `ACC 명중 보정 / PEN 장갑 파훼 / DMG 피해 증폭 / CTRL 거리 제어`로
  표시하고, 좁은 탄창에는 ACC/PEN/DMG/CTRL 축약 배지를 적용했다.
- 기본탄은 전술 전문축과 분리해 `[기본탄] 고명중·고관통·고화력` 성향으로 표시했다.
- 기존 `chain` ID·경로는 보존하면서 연쇄탄을 `장약 증폭탄`으로 전환했다.
- 장약 증폭탄이 유효 적중하면 다음 1발 DMG +2를 부여하고, 도탄 시 미발동,
  다음 격발 소비, 리로드·교전 초기화 소멸, 게이트 통과 후 크리티컬 전 가산을 적용했다.
- 카드·가방·실린더·드래프트·갤러리·정비 UI와 `탄환 전문축 QA` 숏컷을 갱신했다.
- GDD, 밸런스 지침, 태스크 트래커, 구현 계획·워크스루와 히스토리 마스터를 동기화했다.
- 사용자 개인 설정 `.claude/settings.local.json`은 작업 커밋에서 제외했다.

### ✅ 검증
- Godot 4.7 전체 자동 검증 **3,431 통과 / 0 실패 / 기존 경보 3**.
- 9총기×13적×기본/환기구 234대진에서 첫 탄창 유효 대응 실패 0.
- CSV↔19개 탄환 리소스, DMG 버프 발동·소비·도탄·미리보기와 주요 UI 회귀 확인.
- `git diff --check` 통과.

### 📅 다음 예정 작업
- 실제 기기에서 `탄환 전문축 QA`를 실행해 전체/축약 배지의 판독성과
  표식→천공→장약 증폭 순서의 체감을 확인한다.
- 수동 결과에 따라 수치 변경보다 카드 여백·색 대비·효과 문구 길이를 먼저 조정한다.

---

## 2026-08-15 (Sat) - 몬스터 편성·격발 템포·조합탄 전문축 구현

### 🎯 목표
- 기능 구현 우선순위에 맞춰 13종 몬스터의 실제 캠페인 편성, 모바일 격발 입력 호흡,
  연발 후열 이월, 조합탄 선택 편향을 한 묶음으로 정리한다.

### 🛠️ 주요 작업
- 일반·변종 9종과 관문 보스 4종의 계층별 편성을 `EnemyRoster` 정본으로 분리하고,
  관리·정점 대표 4체 QA와 동거리 편성 슬롯을 추가했다.
- #019 다수 적 편성이 일렬로 보이지 않던 문제를 논리 거리와 화면 슬롯 분리로 수정했다.
- 전술 기관단총·제압형 연발에서 앞 적 처치 후 남은 탄이 다음 최근접 적에게 이월되도록
  타깃 재선정 계약을 회귀 고정했다.
- 연출 중 발사·빼내기·재장전 재입력을 차단하고, 총기별 최소 판독 시간과
  Tempo/Suppressor 내부 연발 간격을 적용했다.
- 플레이 로그에서 기본탄 편중과 동일 운용 반복을 확인하고, 기존 운용 분류와 별도로
  화력/관통/명중/제어 `specialty`를 19종 탄환 데이터에 도입했다.
- 카드·가방·실린더·드래프트·갤러리 배지와 툴팁을 전문축 중심으로 개편하고,
  교대탄·인터럽터·만능 약실·리듬 챔버 판정을 전문축 기준으로 전환했다.
- 소총 시작 패키지를 천공+연쇄, 산탄을 파쇄+충격으로 확정하고
  개발자 메뉴에 `탄환 전문축 QA` 진입점을 추가했다.

### ✅ 검증
- Godot 4.7 전체 자동 검증 **3,424 통과 / 0 실패 / 기존 경보 3**.
- 86개 스크립트 컴파일 및 4개 씬 로드 성공.
- 9총기×13적×기본/환기구 234대진에서 첫 탄창 유효 대응 순서 존재 확인.
- CSV↔19개 탄환 리소스, 전문축 분포, 연발 이월, 입력 잠금, UI 배지와 문서 드리프트 검증 완료.
- `git diff --check` 통과.

### 📅 다음 예정 작업
- 2026-08-16 수동 플레이에서 개발자 메뉴의 `탄환 전문축 QA`를 실행한다.
- 명중→관통→화력 순서와 동일 전문축 반복이 각각 인터럽터/교대탄과 리듬 챔버로
  분명히 읽히는지 확인한다.
- 소총의 흡수체 대응과 산탄의 파쇄→공격 순서, 카드 배지 가독성을 실제 기기에서 확인한다.
- 플레이 로그를 남겨 특정 전문축만 다시 반복 선택되는지 재분석한다.
- 현재 변경은 커밋·푸시하지 않은 상태이므로 수동 확인 후 함께 정리한다.
