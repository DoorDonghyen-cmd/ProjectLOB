...
**비장한 절차와 고독**의 무드, 그리고 전술적 피아 식별을 위해 **미니멀 16-bit 레트로 픽셀 아트(Minimalist 16-bit Pixel Art)**를 기본 스타일로 채택합니다. 외곽선 없이 명확한 실루엣 면과 플랫한 2톤 명암, 극도로 제한된 뮤트 톤 팔레트가 핵심입니다.

> ⚠️ **폐기된 방향(되살리지 말 것)**: 테라-호러 · 감염/좀비 · 특수작전 요원. <!-- drift-allow -->
> 호러는 예측 불가능성이 본작의 결정론 정체성과 충돌하므로 **명시적으로 기각**되었습니다.

### 1) 캐릭터 및 크리처 비례 (Minimalist 5 Head-Tall Proportions)
* **주인공 (개조받지 못한 인간)**: **5등신의 미니멀 리얼 비례**를 적용합니다. 전술 장비가 아니라 **주워 맞춘 옷차림**으로, 우측을 겨냥하는 사이드 프로필 실루엣. 그레이·다크 그린 중심의 제한된 뮤트 톤과 플랫한 2톤 명암. **검은색 외곽선 금지.**
  * 게임 전체에서 **인간 실루엣이 온전하게 유지되는 유일한 캐릭터**입니다. 이 대비가 세계관 전달의 핵심이므로 실루엣을 훼손하는 장식을 더하지 마십시오.
* **적 (기계화 개체)**: 기계화(사이보그화)가 곧 계급인 세계이며, 적은 **기계화 비용을 지불한 존재들**입니다. 위층으로 갈수록 기계화율이 높아집니다.
  * ⚠️ **실루엣 붕괴 규칙(필수)**: 기계화율에 따라 인간 실루엣이 무너지며, **아키타입마다 붕괴 방향이 다릅니다** — 돌격=사족형 / 방패=벽 / 회피=흔들림 / 술사=부유체 / 앱소버=구체 / 스크램블러=드론.
  * 전 개체를 인간형으로 그리면 **색상으로만 구분되는 가독성 결함**이 발생합니다. 미니멀 플랫 화풍에서는 치명적입니다.
  * 색은 소속 계층 팔레트를 따릅니다: 침전 갈색 → 공역 주황 → 정비 청록 → 관리 청백 → 정점 자홍.
